...
void uiTask(void *pvParameters) {
    ...
    while(1) {
        ...
        if (millis() - last_imu_poll > 50) {
            ...
            updateIMUVars(imu.accel.x, imu.accel.y, imu.accel.z, imu.gyro.x, imu.gyro.y, imu.gyro.z);
        }
        ...
        if (M5Cardputer.Keyboard.isChange() && M5Cardputer.Keyboard.isPressed()) {
            ...
            if (st.ctrl && !st.shift) {
                if (M5Cardputer.Keyboard.isKeyPressed('a') || M5Cardputer.Keyboard.isKeyPressed('A')) syncPatchToBLE();
                if (M5Cardputer.Keyboard.isKeyPressed('r') || M5Cardputer.Keyboard.isKeyPressed('R')) sendBLECombo('r');
                ...
                if (bankChanged) {
                    ...
                    if (is_sync_master) { broadcastCode(input_buffer, rpn_mode, current_play_mode == MODE_FLOATBEAT); broadcastPlay(0); }
                    syncPatchToBLE();
                } else {
                    if (M5Cardputer.Keyboard.isKeyPressed('z') || M5Cardputer.Keyboard.isKeyPressed('Z')) { 
                        int prev = (undo_ptr - 1 + UNDO_DEPTH) % UNDO_DEPTH; 
                        if (undo_stack[prev] != "") { 
                            undo_ptr = prev; input_buffer = undo_stack[undo_ptr]; cursor_pos = input_buffer.length(); sendBLECombo('z');
                        } 
                    }
            ...
            // 4. FN COMMANDS
                ...
                for (auto c : st.word) {
                    ...
                    if (c == 'm' || c == 'M') {
                        ...
                        if (!is_sync_initialized) { initESPNowSync(); is_sync_initialized = true; }
                        if (!is_sync_master) {
                            is_sync_master = true; broadcastPlay(t_raw); 
