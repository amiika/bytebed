...
const PresetConfig defaultBanks[10][10] = {
    ...
    // BANK 1: CLASSIC (8kHz Bytebeat)
    {
        ...
        {"t*[0,1,1.2,4/3,3/2,8/5,16/9,2]['110033003333004455003300330030504400220022004400550033003300303011003300333300445500330033003355777766665050404055003300333300557700770066005500444422332200202060606060505040405500330033333343'[t>>9]]*6", 8000, MODE_BYTEBEAT},
    ...
    // BANK 3: FLOATBEAT (48kHz Floatbeat)
    {
        ...
        {"e=256,r=40,d=32,p=t*2**([0,12,12,0,12,12,5,7][7&t>>13]/12)/2.43,ev=r-t/e%d,sin(sin(sin(p*PI/16)/76*ev+p*PI/256)/8*ev+p*PI/256)*ev/64", 48000, MODE_FLOATBEAT}, 
    ...
    // BANK 4: FLOATBEAT SUM & REDUCE PATTERNS
    {
        ...
        {"p=1000/(t%123456),sum(12,i=>s(t*p*(i*2+1))/(i*2)) // Wait for it ...", 48000, MODE_FLOATBEAT},                                               
